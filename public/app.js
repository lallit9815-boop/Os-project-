...
/**
 * Generate allocation sequence by method.
 * Returns an ordered array of unique block indexes.
 */
function generateAllocations(method, blocks, files) {
  const safeBlocks = Math.max(1, Number.isFinite(blocks) ? Math.floor(blocks) : 1);
  const safeFiles = Math.max(1, Number.isFinite(files) ? Math.floor(files) : 1);
  const needed = Math.min(safeBlocks, safeFiles);

  if (method === 'Contiguous Allocation') {
    const start = Math.max(0, Math.floor((safeBlocks - needed) / 2));
    return Array.from({ length: needed }, (_, i) => start + i);
  }

  if (method === 'Linked Allocation') {
    // Build a deterministic non-contiguous chain without infinite loops.
    const pool = Array.from({ length: safeBlocks }, (_, i) => i);
    const chain = [];
    let cursor = 0;

    for (let i = 0; i < needed; i += 1) {
      cursor = (cursor + 3) % pool.length;
      chain.push(pool.splice(cursor, 1)[0]);
  ...
  // Indexed Allocation (first block is index block, remainder scattered).
  const allocations = [0];
  const used = new Set(allocations);
  let candidate = 2;
  while (allocations.length < needed) {
    const next = candidate % safeBlocks;
    if (!used.has(next)) {
...
function HomePage({ setCurrent }) {
  return (
    <section className="container hero glass">
      ...
      <div className="glass feature-tile" style={{ padding: '22px' }}>
...
function SimulationPage() {
  ...
  const allocationPlan = useMemo(
    () => generateAllocations(method, safeBlocks, safeFiles),
    [method, safeBlocks, safeFiles]
  );
  ...
  async function saveSimulation() {
    try {
      await fetch('/api/simulations', {
        ...
        body: JSON.stringify({ method, blocks: safeBlocks, files: safeFiles, allocations: allocationPlan }),
      });
    } catch (_e) {
  ...
  function handleStart() {
    if (!Number.isFinite(blocks) || !Number.isFinite(files) || blocks < 1 || files < 1) {
      setError('Blocks and files must be positive numbers.');
      return;
    }
    if (safeFiles > safeBlocks) {
